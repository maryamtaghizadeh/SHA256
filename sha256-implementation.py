...
def sha256(message):
    ...
    def preprocess(message):
        ...
        bit_string = ''.join(format(ord(c), '08b') for c in message)
        
        # Pad the message
        bit_string += '1'
        while len(bit_string) % 512 != 448:
            bit_string += '0'
        bit_string += format(len(message) * 8, '064b')
        
        return bit_string

    def chunk_message(bit_string):
        return [bit_string[i:i+512] for i in range(0, len(bit_string), 512)]
    ...
    def compress(a, b, c, d, e, f, g, h, w, k):
        for i in range(64):
    ...
    chunks = chunk_message(bit_string)
